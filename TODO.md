...
- [x] Extract `EchoService` to `src/Olve.Template.Api/Echo/EchoService.cs`, refactor `/echo` endpoint to a lambda
- [x] Split tests into `test/Olve.Template.Api.UnitTests/` (unit tests for EchoService) and `test/Olve.Template.Api.IntegrationTests/` (existing WebApplicationFactory tests)
...
- [x] Set up C# client generation with Refitter (source gen from `api.json`) in `clients/Olve.Template.Api.Client/`
- [x] Set up TS client generation with Kiota (dotnet tool from `api.json`) in `clients/olve-template-api-client-ts/`
- [x] Add `.editorconfig`
